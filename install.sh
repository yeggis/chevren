...
detect_distro() {
  ...
  *)
    ...
    if echo "$DISTRO_ID_LIKE" | grep -q "arch"; then
      PKG_FAMILY="arch"
    elif echo "$DISTRO_ID_LIKE" | grep -q "debian"; then
      PKG_FAMILY="debian"
      ...
      die "Tanınmayan distro: $DISTRO_ID. Lütfen issue açın: https://github.com/kullanici/chevren/issues"
    fi
    ;;
  esac
...
pkg_install_arch() { maybe_sudo pacman -S --needed --noconfirm "$@"; }
pkg_install_debian() { maybe_sudo apt-get install -y "$@"; }
pkg_install_fedora() { maybe_sudo dnf install -y "$@"; }
pkg_install_opensuse() { maybe_sudo zypper install -y "$@"; }
...
update_repos() {
  ...
  debian) maybe_sudo apt-get update -qq ;;
  fedora) maybe_sudo dnf check-update -q || true ;;
  opensuse) maybe_sudo zypper refresh -q ;;
  esac
